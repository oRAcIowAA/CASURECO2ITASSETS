...
return new class extends Migration 
{
    ...
    public function up(): void
    {
        if (DB::getDriverName() === 'sqlite') {
            ...
            Schema::table('pc_histories', function (Blueprint $table) {
                $table->enum('action', ['assigned', 'returned', 'transferred', 'reassigned', 'condemned', 'defective', 'disposed', 'repaired', 'restored'])->default('assigned');
            });
        }
        else {
            DB::statement("ALTER TABLE pc_histories MODIFY COLUMN action ENUM('assigned', 'returned', 'transferred', 'reassigned', 'condemned', 'defective', 'disposed', 'repaired', 'restored') NOT NULL DEFAULT 'assigned'");
        }
    }
