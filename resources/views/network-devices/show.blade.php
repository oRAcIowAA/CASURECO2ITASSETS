                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                        <div class="flex items-start w-full sm:w-auto">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600 font-bold tracking-wide">{{ strtoupper($networkDevice->brand) }}
                                    {{ strtoupper($networkDevice->model) }}</p>

                                @if($networkDevice->updatedBy)
                                    <div class="mt-1 text-xs text-gray-500">
                                        <span class="font-medium text-gray-700">Last updated by:</span>
                                        {{ $networkDevice->updatedBy->name }}
                                    </div>
                                @endif
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 mb-6 pt-4 border-t border-gray-100">
                        ...
                        @if(strtolower($networkDevice->status) === 'disposed')
                            <div x-data="{ showRestoreModal: false }" class="inline w-full sm:w-auto">
                                <button type="button" @click="showRestoreModal = true"
                                    class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-purple-200 text-sm font-medium rounded-md text-purple-700 bg-purple-50 hover:bg-purple-100">
                                    <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div x-show="showRestoreModal" class="fixed inset-0 z-50 overflow-y-auto"
                                    ...
                                    <div
                                        ...
                                        <div x-show="showRestoreModal" x-transition:enter="ease-out duration-300"
                                            ...
                                            <form method="POST"
                                                action="{{ route('network-devices.restore', $networkDevice) }}">
                                                @csrf
                        ...
                        @else
                            @if(!in_array(strtolower($networkDevice->status), ['condemned', 'disposed']))
                                <a href="{{ route('network-devices.transfer', $networkDevice) }}"
                            ...
                            @endif
                            <a href="{{ route('network-devices.dispose', $networkDevice) }}"
                                ...
                                @php $statusStr = strtolower($networkDevice->status); @endphp
                            ...
                            @if(strtolower($networkDevice->status) === 'defective')
                                <form method="POST" action="{{ route('network-devices.repair', $networkDevice) }}"
                        ...
                        <a href="{{ route('network-devices.print-label', $networkDevice) }}" target="_blank"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ ucfirst($networkDevice->device_type) }}
                            </p>
                        </div>
                        ...
                        @if($networkDevice->device_type == 'switch')
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">{{ ucfirst($networkDevice->switch_type) }}
                                </p>
                            </div>
                        @endif
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ ucfirst($networkDevice->network_speed) }}
                            </p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Date Issued</label>
                            <p class="text-lg font-semibold text-gray-900">
                                {{ $networkDevice->date_issued ? $networkDevice->date_issued->format('M d, Y') : 'N/A' }}
                            </p>
                        </div>
                        ...
                        <div class="border-t border-gray-200 pt-6 mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <p class="text-lg font-semibold text-gray-900">{{ $networkDevice->ip_address ?? 'N/A' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="border-t border-gray-200 pt-6 mb-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            @php
                                $source = $networkDevice->employee_id ? $networkDevice->employee : $networkDevice;
                            @endphp
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1 uppercase">Location <span
                                        class="text-[10px] text-indigo-500 font-bold uppercase tracking-tight">{{ $networkDevice->employee_id ? '(BASED ON EMPLOYEE)' : '' }}</span></label>
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">
                                    {{ $networkDevice->employee_id ? strtoupper($networkDevice->employee->full_name) : 'UNASSIGNED' }}
                                    @if($networkDevice->employee)
                                        <span
                                            class="text-sm text-gray-500">({{ strtoupper($networkDevice->employee->position) }})</span>
                                    @endif
                                </p>
                            </div>
                            <div>
                    ...
                    @if($networkDevice->employee_id)
                        <form method="POST" action="{{ route('network-devices.return', $networkDevice) }}"
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <h3 class="text-sm font-medium text-yellow-800">This device is currently assigned to
                                            {{ $networkDevice->employee->full_name }}</h3>
                                        <div class="mt-2">
                                            <button type="submit"
