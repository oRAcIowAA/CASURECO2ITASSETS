<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-red-600 leading-tight">
            {{ __('Condemn/Dispose Network Device: ') . $networkDevice->brand . ' ' . $networkDevice->model }}
        </h2>
    </x-slot>
                ...
                <div class="p-6">
                    ...
                    <div class="mb-6">
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            Device: <strong>{{ $networkDevice->brand ?? 'Unknown' }} {{ $networkDevice->model ?? 'Unknown' }}</strong><br>
                            Current Status: {{ ucfirst($networkDevice->status) }}
                        </p>
                    </div>

                    <form method="POST" action="{{ route('network-devices.condemn', $networkDevice) }}">
                        @csrf
                        ...
                        <div class="flex items-center justify-between mt-6">
                            ...
                            <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
                                {{ strtolower($networkDevice->status) === 'condemned' ? 'Finalize Archive' : 'Confirm' }}
                            </button>
                        </div>
                    </form>
                </div>
