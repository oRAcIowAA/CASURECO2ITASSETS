        ...
        <div class="w-full sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-gray-500 uppercase font-semibold">TOTAL: {{ $networkDevices->total() }} UNITS
                    </p>
                </div>
                <!-- Add New Button -->
            ...
            <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200 shadow-sm">
                <form method="GET" action="{{ route('network-devices.index') }}"
                    class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4" x-data="{ 
                    ...
                    <div class="col-span-1 md:col-span-3 lg:col-span-6 flex items-center gap-2">
                        ...
                        @if(request('status') === 'Available')
                            <a href="{{ route('network-devices.index', request()->except('status')) }}"
                                class="ml-2 text-indigo-600 hover:text-indigo-900 text-xs font-bold uppercase">CLEAR
                                FILTER</a>
                        @endif
                    </div>
                </form>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600">
                                        <a href="{{ route('network-devices.show', $device) }}"
                                            class="hover:underline">{{ $device->asset_tag }}</a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 uppercase tracking-wide">
                                            {{ $device->device_type }}</div>
                                        <div class="text-xs text-gray-500">{{ $device->brand }} {{ $device->model }}
                                            @if($device->network_ports) ({{ $device->network_ports }} Ports) @endif</div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <form action="{{ route('network-devices.destroy', $device) }}" method="POST"
