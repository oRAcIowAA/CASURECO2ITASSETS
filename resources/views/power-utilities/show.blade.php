<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Power Utility Details: ') . $powerUtility->asset_tag }}
        </h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div
                    class="mb-6 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded font-bold text-xs uppercase">
                    {{ session('success') }}
                </div>
            @endif
                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                        <div class="flex items-start w-full sm:w-auto">
                            ...
                            <div>
                                <h1 class="text-2xl font-bold text-gray-900 uppercase">{{ $powerUtility->asset_tag }}
                                </h1>
                                <p class="text-gray-600 font-bold tracking-wide uppercase">{{ $powerUtility->type }} -
                                    {{ $powerUtility->brand }} {{ $powerUtility->model }}</p>

                                @if($powerUtility->updatedBy)
                                    <div class="mt-1 text-xs text-gray-500">
                                        <span class="font-medium text-gray-700">Last updated by:</span>
                                        {{ $powerUtility->updatedBy->name }}
                                    </div>
                                @endif

                                <!-- Status Badge -->
                                <div class="mt-3">
                                    @php $statusStr = strtolower($powerUtility->status); @endphp
                        ...
                        <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto mt-4 sm:mt-0">
                            <a href="{{ route('power-utilities.edit', $powerUtility) }}"
                            ...
                            <a href="{{ route('power-utilities.index') }}"
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 mb-6 pt-4 border-t border-gray-100">
                        @if($statusStr === 'disposed')
                            <div x-data="{ showRestoreModal: false }" class="inline w-full sm:w-auto">
                                <button type="button" @click="showRestoreModal = true"
                                    class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-indigo-200 text-sm font-semibold rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100">
                                ...
                                <div x-show="showRestoreModal" class="fixed inset-0 z-50 overflow-y-auto"
                                    ...
                                    <div
                                        ...
                                        <div x-show="showRestoreModal" x-transition:enter="ease-out duration-300"
                                            ...
                                            <form method="POST"
                                                action="{{ route('power-utilities.restore', $powerUtility) }}">
                                                @csrf
                                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                    <div class="sm:flex sm:items-start">
                                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                                            <h3 class="text-lg leading-6 font-bold text-gray-900 uppercase"
                                                                id="modal-title">Restore Disposed Data</h3>
                                                            <div class="mt-2">
                                                                <p class="text-sm text-gray-500 mb-4 uppercase">Please
                                                                    confirm your identity to restore this permanently
                                                                    archived device.</p>
                                                                <div class="mb-4">
                                                                    <label for="password"
                                                                        class="block text-sm font-bold text-gray-700 uppercase">Your
                                                                        Password</label>
                                                                ...
                                                                <div>
                                                                    <label for="reason"
                                                                        class="block text-sm font-bold text-gray-700 uppercase">Reason
                                                                        for Restore</label>
                                                                    <input type="text" name="reason" id="reason"
                                                                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md uppercase font-bold"
                                                                        placeholder="e.g., Accidental disposal..." required>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div
                                                    class="bg-gray-50 px-4 py-3 sm:px-6 flex justify-end gap-3 font-bold uppercase">
                        ...
                        @else
                            ...
                            <a href="{{ route('power-utilities.dispose', $powerUtility) }}"
                                class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-red-200 text-sm font-semibold rounded-md text-red-700 bg-red-50 hover:bg-red-100">
                                <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                                    </path>
                            ...
                            @if($statusStr === 'defective')
                                <form method="POST" action="{{ route('power-utilities.repair', $powerUtility) }}"
                                    class="inline w-full sm:w-auto">
                                    @csrf
                                    <button type="submit"
                                        class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-green-200 text-sm font-medium rounded-md text-green-700 bg-green-50 hover:bg-green-100 font-bold"
                        ...
                        <a href="{{ route('power-utilities.print-label', $powerUtility) }}" target="_blank"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <p class="text-lg font-bold text-gray-900 uppercase">{{ $powerUtility->type }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Brand / Model</label>
                            <p class="text-lg font-bold text-gray-900 uppercase">{{ $powerUtility->brand }}
                                {{ $powerUtility->model }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Serial Number</label>
                            <p class="text-lg font-bold text-gray-900 uppercase font-mono tracking-tight">
                                {{ $powerUtility->serial_number ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Capacity (VA)</label>
                            <p class="text-lg font-bold text-gray-900 uppercase">{{ $powerUtility->capacity ?? 'N/A' }}
                            </p>
                        </div>
                        @if($powerUtility->type === 'AVR')
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1 uppercase">Input Voltage</label>
                                <p class="text-lg font-bold text-gray-900 uppercase">
                                    {{ $powerUtility->input_voltage ?? 'N/A' }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1 uppercase">Output Voltage</label>
                                <p class="text-lg font-bold text-gray-900 uppercase">
                                    {{ $powerUtility->output_voltage ?? 'N/A' }}</p>
                            </div>
                        @endif
                        <div>
                    ...
                    @if($powerUtility->remarks)
                        <div class="border-t border-gray-100 pt-4 mb-6">
                            <label class="block text-sm font-medium text-gray-500 mb-1 uppercase">Remarks</label>
                            <p class="text-gray-900 whitespace-pre-wrap uppercase text-sm font-bold">
                                {{ $powerUtility->remarks }}</p>
                        </div>
                    @endif
                    ...
                    <div class="border-t border-gray-200 pt-6 mb-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-bold text-gray-900 uppercase">{{ $source->department ?? 'N/A' }}
                                </p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Division</label>
                                <p class="text-lg font-bold text-gray-900 uppercase">{{ $source->division ?? 'N/A' }}
                                </p>
                            </div>
                            <div>
                    ...
                    <div class="mt-6 border-t border-gray-100 pt-6">
                        @if(!$powerUtility->employee_id && !in_array($statusStr, ['disposed', 'condemned']))
                        ...
                        @elseif($powerUtility->employee_id)
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-sm font-bold text-indigo-800 uppercase">
                                            This device is currently assigned to {{ $powerUtility->employee->full_name }}.
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            <!-- Assignment History -->
            @if($powerUtility->history->count() > 0)
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 uppercase">
                                @foreach($powerUtility->history->sortByDesc('created_at') as $history)
                                    <tr class="font-bold">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @endphp
                                            <span
                                                class="px-2 py-0.5 text-[10px] leading-5 font-bold rounded-full {{ $colorClass }}">
