<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Transfer Power Utility: ') . $powerUtility->type . ' ' . $powerUtility->brand . ' ' . $powerUtility->model }}
        </h2>
    </x-slot>
                ...
                <div class="p-6">
                    ...
                    <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <p class="text-gray-600 mt-1">
                            @if($powerUtility->employee)
                                Currently assigned to: <strong>{{ $powerUtility->employee->full_name }}</strong> 
                                ({{ strtoupper($powerUtility->employee->department ?? 'N/A') }} / {{ strtoupper($powerUtility->employee->division ?? 'N/A') }} / {{ strtoupper($powerUtility->employee->group ?? 'N/A') }})
                            @else
                    ...
                    <form method="POST" action="{{ route('power-utilities.reassign', $powerUtility) }}">
                        @csrf
                        ...
                        <div class="mb-6" x-data>
                            ...
                            <select name="employee_id" id="employee_id" x-init="new Choices($el, { searchPlaceholderValue: 'SEARCH NAME, DEPARTMENT...' })" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                                ...
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ $powerUtility->employee_id == $employee->id ? 'disabled' : '' }}>
                                        {{ strtoupper($employee->full_name) }} &mdash; {{ strtoupper($employee->department ?? 'N/A') }} / {{ strtoupper($employee->division ?? 'N/A') }}
                                        {{ $powerUtility->employee_id == $employee->id ? '(CURRENT OWNER)' : '' }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-6">
                            <a href="{{ route('power-utilities.show', $powerUtility) }}" class="text-gray-600 hover:text-gray-800 font-bold uppercase text-sm">
                                Cancel
                            </a>
                            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 font-bold uppercase text-sm">
                                Confirm Transfer
                            </button>
                        </div>
                    </form>
