<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Network Device Assignment History: ') . $networkDevice->asset_tag }}
        </h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                <a href="{{ route('network-devices.show', $networkDevice) }}" 
                   class="inline-flex items-center text-gray-600 hover:text-gray-800 font-medium">
                    ← Back to Network Device Details
                </a>
                ...
                <div class="p-6">
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600">{{ $networkDevice->brand }} {{ $networkDevice->model }}</p>
                            <div class="mt-2">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium 
                                    @if(strtolower($networkDevice->status) == 'available') bg-green-100 text-green-800 @elseif(strtolower($networkDevice->status) == 'disposed') bg-red-100 text-red-800 @else bg-yellow-100 text-yellow-800 @endif">
                                    {{ ucfirst(str_replace('_', ' ', $networkDevice->status)) }}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <div class="px-6 py-8 text-center text-gray-500">
                    No assignment history available for this network device.
                </div>
                @endif
            </div>
        </div>
