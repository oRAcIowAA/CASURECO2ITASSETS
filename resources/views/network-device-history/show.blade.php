<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('History Log: ') . $networkDevice->brand . ' ' . $networkDevice->model }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4 flex justify-between items-center">
                <a href="{{ route('network-devices.show', $networkDevice) }}" class="text-indigo-600 hover:text-indigo-900">
                    ...
                    <div class="mb-6">
                        ...
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2 lg:grid-cols-4 mt-2">
                            <div class="sm:col-span-1">
                                ...
                                <dd class="mt-1 text-sm text-gray-900">{{ $networkDevice->brand }} {{ $networkDevice->model }}</dd>
                            </div>
                            <div class="sm:col-span-1">
                                <dt class="text-sm font-medium text-gray-500">Current Status</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ ucfirst($networkDevice->status) }}</dd>
                            </div>
                            <div class="sm:col-span-1">
                                <dt class="text-sm font-medium text-gray-500">Current Owner</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $networkDevice->employee->full_name ?? 'Unassigned' }}</dd>
                            </div>
                        </dl>
                    </div>
