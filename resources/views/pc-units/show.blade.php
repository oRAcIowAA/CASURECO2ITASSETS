<x-app-layout>
                ...
                <div class="p-6">
                                ...
                                @if(strtolower($pcUnit->device_type) == 'laptop')
                                    <svg class="w-8 h-8 text-blue-600" fill="none" stroke="currentColor"
                                        ...
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3 12h18M5 18h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2zM9 21h6">
                                        </path>
                                    </svg>
                                @else
                                    <svg class="w-8 h-8 text-blue-600" fill="none" stroke="currentColor"
                            ...
                            <div>
                                ...
                                @if($pcUnit->updatedBy)
                                    <div class="mt-1 text-xs text-gray-500">
                                        <span class="font-medium text-gray-700">Last updated by:</span>
                                        {{ $pcUnit->updatedBy->name }}
                                    </div>
                                @endif

                                <!-- Status Badge -->
                                <div class="mt-3">
                                    @if($pcUnit->status == 'assigned')
                                        <span
                                    ...
                                    @elseif($pcUnit->status == 'available')
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                                            <svg class="-ml-1 mr-1 h-4 w-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                    ...
                                    @elseif($pcUnit->status == 'defective')
                                        <span
                                    ...
                                    @elseif($pcUnit->status == 'condemned')
                                        <span
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 mb-6 pt-4 border-t border-gray-100">
                        <a href="{{ route('pc-history.show-by-pc', $pcUnit->id) }}"
                            class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-blue-200 text-sm font-medium rounded-md text-blue-700 bg-blue-50 hover:bg-blue-100">
                        ...
                        @if(strtolower($pcUnit->status) === 'disposed')
                            <div x-data="{ showRestoreModal: false }" class="inline w-full sm:w-auto">
                                <button type="button" @click="showRestoreModal = true"
                                    class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-indigo-200 text-sm font-medium rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100">
                                    <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <!-- Restore Modal -->
                                <div x-show="showRestoreModal" class="fixed inset-0 z-50 overflow-y-auto"
                                    style="display: none;" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                                    <div
                                        class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                        <div x-show="showRestoreModal" @click="showRestoreModal = false"
                                        ...
                                        <div x-show="showRestoreModal" x-transition:enter="ease-out duration-300"
                                            ...
                                            <form method="POST" action="{{ route('pc-units.restore', $pcUnit) }}">
                                                @csrf
                                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                    <div class="sm:flex sm:items-start">
                                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                                            <h3 class="text-lg leading-6 font-medium text-gray-900"
                                                                id="modal-title">Restore Disposed Data</h3>
                                                            <div class="mt-2">
                                                                <p class="text-sm text-gray-500 mb-4">Please confirm your
                                                                    identity to restore this permanently archived device.
                                                                </p>
                                                                <div class="mb-4">
                                                                    <label for="password"
                                                                        class="block text-sm font-medium text-gray-700">Your
                                                                        Password</label>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <input type="text" name="reason" id="reason"
                                                                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                                                        placeholder="e.g., Accidental disposal, Unit was recovered..."
                                                                        required>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="bg-gray-50 px-4 py-3 sm:px-6 flex justify-end gap-3">
                                                    <button type="button" @click="showRestoreModal = false"
                                                        class="w-full sm:w-28 inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm">
                                                        Cancel
                                                    </button>
                                                    <button type="submit"
                                                        class="w-full sm:w-28 inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm">
                                                        Restore
                                                    </button>
                                                </div>
                                            </form>
                        ...
                        @else
                            @if(!in_array(strtolower($pcUnit->status), ['condemned', 'disposed']))
                                <a href="{{ route('pc-units.transfer', $pcUnit) }}"
                                    class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-indigo-200 text-sm font-medium rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100 font-bold">
                                    <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @endif
                            <a href="{{ route('pc-units.dispose', $pcUnit) }}"
                                class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-red-200 text-sm font-medium rounded-md text-red-700 bg-red-50 hover:bg-red-100">
                                <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                                    </path>
                                </svg>
                                @php $statusStr = strtolower($pcUnit->status); @endphp
                            ...
                            @if(strtolower($pcUnit->status) === 'defective')
                                <form method="POST" action="{{ route('pc-units.repair', $pcUnit) }}"
                                    class="inline w-full sm:w-auto">
                                    @csrf
                                    <button type="submit"
                                        class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-green-200 text-sm font-medium rounded-md text-green-700 bg-green-50 hover:bg-green-100"
                                        onclick="return confirm('Mark this device as repaired?');">
                                        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <a href="{{ route('pc-units.print-label', $pcUnit) }}" target="_blank"
                            class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-gray-200 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ strtoupper($pcUnit->device_type) }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Model</label>
                            <p class="text-lg font-semibold text-gray-900">{{ strtoupper($pcUnit->model) }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Serial Number</label>
                            <p class="text-lg font-semibold text-gray-900">
                                {{ strtoupper($pcUnit->serial_number ?? 'N/A') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Operating System</label>
                            <p class="text-lg font-semibold text-gray-900">
                                {{ strtoupper($pcUnit->os_version ?? 'N/A') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Processor</label>
                            <p class="text-lg font-semibold text-gray-900">{{ strtoupper($pcUnit->processor ?? 'N/A') }}
                            </p>
                        </div>
                        <div>
                    ...
                    <div class="border-t border-gray-200 pt-6 mb-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Monitor Information</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Monitor Brand</label>
                                <p class="text-lg font-semibold text-gray-900">
                                    {{ strtoupper($pcUnit->monitor_brand ?? 'N/A') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Monitor Serial
                                    Number</label>
                                <p class="text-lg font-semibold text-gray-900 uppercase font-mono">
                                    {{ $pcUnit->monitor_serial ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- MS Office Details -->
                    <div class="border-t border-gray-200 pt-6 mb-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">MS Office Information</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Status</label>
                                <p class="text-lg font-semibold text-gray-900">
                                    {{ strtoupper($pcUnit->ms_office_licensed ?? 'N/A') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Version</label>
                                <p class="text-lg font-semibold text-gray-900">
                                    {{ strtoupper($pcUnit->ms_office_version ?? 'N/A') }}</p>
                            </div>
                            @if(strtoupper($pcUnit->ms_office_licensed) === 'LICENSED')
                                <div>
                                    <label
                                        class="block text-sm font-medium text-gray-500 mb-1 text-indigo-600">Email/Account</label>
                                    <p class="text-lg font-semibold text-gray-900">{{ $pcUnit->ms_office_email ?? 'N/A' }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-sm font-medium text-gray-500 mb-1 text-indigo-600">Password</label>
                                    <p class="text-lg font-semibold text-gray-900">
                                        {{ $pcUnit->ms_office_password ?? 'N/A' }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900 font-mono">
                                    {{ $pcUnit->mac_address ?? 'N/A' }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Network Segment</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $pcUnit->network_segment ?? 'N/A' }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-6 mb-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1 uppercase">Department</label>
                                <p class="text-lg font-semibold text-gray-900 uppercase">
                                    {{ $source->department ?? 'N/A' }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1 uppercase">Division</label>
                                <p class="text-lg font-semibold text-gray-900 uppercase">
                                    {{ $source->division ?? 'N/A' }}</p>
                            </div>
                            <div>
                    ...
                    <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                        <div class="flex items-start">
                            ...
                            <div class="ml-3 flex-1">
                                <h3 class="text-sm font-medium text-yellow-800">This PC is currently assigned to
                                    {{ $pcUnit->employee->full_name }}</h3>
                                <div class="mt-2">
                                    <button type="submit"
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($pcUnit->history->sortByDesc('created_at') as $history)
                                    <tr>
    ...
    @if(session('print_disposal'))
        <script>
            window.open('{{ route('pc-units.print-disposal', $pcUnit) }}', '_blank');
        </script>
    @endif
</x-app-layout>
