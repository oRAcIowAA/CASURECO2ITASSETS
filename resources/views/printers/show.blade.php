            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden mb-6">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                        <div class="flex items-start w-full sm:w-auto">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600 font-bold tracking-wide">{{ strtoupper($printer->brand) }} {{ strtoupper($printer->model) }}</p>

                                @if($printer->updatedBy)
                                    <div class="mt-1 text-xs text-gray-500">
                                        <span class="font-medium text-gray-700">Last updated by:</span> {{ $printer->updatedBy->name }}
                                    </div>
                                @endif
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 mb-6 pt-4 border-t border-gray-100">
                        ...
                        @if(strtolower($printer->status) === 'disposed')
                            <div x-data="{ showRestoreModal: false }" class="inline w-full sm:w-auto">
                                <button type="button" @click="showRestoreModal = true" class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-indigo-200 text-sm font-medium rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100">
                        ...
                        @else
                            <a href="{{ route('printers.transfer', $printer) }}" class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-indigo-200 text-sm font-medium rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100">
                            ...
                            <a href="{{ route('printers.dispose', $printer) }}" class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-red-200 text-sm font-medium rounded-md text-red-700 bg-red-50 hover:bg-red-100">
                            ...
                            <form method="POST" action="{{ route('printers.repair', $printer) }}" class="inline w-full sm:w-auto">
                                @csrf
                        ...
                        <a href="{{ route('printers.print-label', $printer) }}" target="_blank" class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-gray-200 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 sm:ml-4">
                    ...
                    <div class="border-t border-gray-200 pt-6 mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">{{ $printer->ip_address ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="border-t border-gray-200 pt-6 mb-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">{{ strtoupper($printer->department ?? 'N/A') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">DIVISION</label>
                                <p class="text-lg font-semibold text-gray-900">{{ strtoupper($printer->division ?? 'N/A') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">GROUP</label>
                                <p class="text-lg font-semibold text-gray-900">{{ strtoupper($printer->group ?? 'N/A') }}</p>
                            </div>
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">
                                    ...
                                    @if($printer->employee)
                                        <span class="text-sm text-gray-500">({{ strtoupper($printer->employee->position) }})</span>
                                    @endif
                                </p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">DATE ASSIGNED</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $printer->date_assigned ? \Carbon\Carbon::parse($printer->date_assigned)->format('M d, Y') : 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    @if($printer->employee_id)
                        <form method="POST" action="{{ route('printers.return', $printer) }}" class="mb-6 border-t border-gray-200 pt-6">
                            @csrf
                            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <h3 class="text-sm font-medium text-yellow-800">This device is currently assigned to {{ $printer->employee->full_name }}</h3>
                                        <div class="mt-2">
                                            <button type="submit" 
