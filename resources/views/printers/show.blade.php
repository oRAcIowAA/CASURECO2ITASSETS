<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Printer Details: ') . $printer->asset_tag }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden mb-6">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                        <div class="flex items-start w-full sm:w-auto">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600 font-bold tracking-wide">{{ strtoupper($printer->type) }} -
                                    {{ strtoupper($printer->brand) }} {{ strtoupper($printer->model) }}</p>

                                @if($printer->updatedBy)
                                    <div class="mt-1 text-xs text-gray-500">
                                        <span class="font-medium text-gray-700">Last updated by:</span>
                                        {{ $printer->updatedBy->name }}
                                    </div>
                                @endif

                                <!-- Status Badge -->
                                <div class="mt-3">
                                    @if(strtolower($printer->status) == 'available')
                                        <span
                                    ...
                                    @elseif(strtolower($printer->status) == 'assigned')
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                                            <svg class="-ml-1 mr-1 h-4 w-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 mb-6 pt-4 border-t border-gray-100">
                        <a href="{{ route('printer-history.show', $printer->id) }}"
                        ...
                        @if(strtolower($printer->status) === 'disposed')
                                ...
                                <div x-show="showRestoreModal" class="fixed inset-0 z-50 overflow-y-auto"
                                    ...
                                    <div
                                        ...
                                        <div x-show="showRestoreModal" x-transition:enter="ease-out duration-300"
                                            ...
                                            <form method="POST" action="{{ route('printers.restore', $printer) }}">
                                                @csrf
                        ...
                        @else
                            @if(!in_array(strtolower($printer->status), ['condemned', 'disposed']))
                                <a href="{{ route('printers.transfer', $printer) }}"
                            ...
                            @endif
                            <a href="{{ route('printers.dispose', $printer) }}"
                                ...
                                @php $statusStr = strtolower($printer->status); @endphp
                            ...
                            @if(strtolower($printer->status) === 'defective')
                                <form method="POST" action="{{ route('printers.repair', $printer) }}"
                        ...
                        <a href="{{ route('printers.print-label', $printer) }}" target="_blank"
                            class="w-full sm:w-auto justify-center inline-flex items-center px-3 py-1.5 border border-gray-200 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 sm:ml-4">
                            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ strtoupper($printer->brand) }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Model</label>
                            <p class="text-lg font-semibold text-gray-900">{{ strtoupper($printer->model) }}</p>
                        </div>
                        @if($printer->serial_number)
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900 uppercase font-mono">
                                    {{ $printer->serial_number }}</p>
                            </div>
                        @endif
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Date Issued</label>
                            <p class="text-lg font-semibold text-gray-900">
                                {{ $printer->date_issued ? $printer->date_issued->format('M d, Y') : 'N/A' }}</p>
                        </div>
                        <!-- Add more fields as needed based on Printer model -->
                    </div>

                    <!-- Network Information -->
                    @if($printer->has_network_port)
                        <div class="border-t border-gray-200 pt-6 mb-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">Network Information</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-medium text-gray-500 mb-1">Network Port</label>
                                    <p class="text-lg font-semibold text-gray-900">Yes</p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-500 mb-1">IP Address / Type</label>
                                    <p class="text-lg font-semibold text-gray-900">
                                        {{ $printer->ip_address ?? 'N/A' }}
                                        <span
                                            class="text-sm font-normal text-gray-500">({{ $printer->ip_type ?? 'Dynamic' }})</span>
                                    </p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-500 mb-1">MAC Address</label>
                                    <p class="text-lg font-semibold text-gray-900">{{ $printer->mac_address ?? 'N/A' }}</p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-500 mb-1">Network Segment</label>
                                    <p class="text-lg font-semibold text-gray-900">{{ $printer->network_segment ?? 'N/A' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    @if($printer->employee_id)
                        <form method="POST" action="{{ route('printers.return', $printer) }}"
                            ...
                            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <h3 class="text-sm font-medium text-yellow-800">This device is currently assigned to
                                            {{ $printer->employee->full_name }}</h3>
                                        <div class="mt-2">
                                            <button type="submit"
                    ...
                    @elseif($printer->status == 'Available')
                        <div class="bg-blue-50 border-l-4 border-blue-400 p-4 rounded mb-6 border-t border-gray-200 mt-6">
                            <div class="flex items-start">
                                ...
                                <div class="ml-3 flex-1">
                                    <h3 class="text-sm font-medium text-blue-800">This device is available for assignment
                                    </h3>
                                </div>
                            </div>
                        </div>
                    ...
                    @php
                        ...
                        $deviceHistory = \App\Models\PrinterHistory::where('printer_id', $printer->id)
    ...
    @if(session('print_disposal'))
        <script>
            window.open('{{ route('printers.print-disposal', $printer) }}', '_blank');
        </script>
    @endif
</x-app-layout>
