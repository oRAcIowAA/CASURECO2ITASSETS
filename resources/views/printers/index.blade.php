    ...
    <div class="py-12">
        <div class="w-full sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                <div>
                        <h3 class="text-lg font-bold text-gray-900 uppercase">ALL PRINTERS</h3>
                        <p class="text-sm text-gray-500 uppercase font-semibold">TOTAL: {{ $printers->total() }} UNITS</p>
                </div>
            ...
            <!-- LIST VIEW -->
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <form method="GET" action="{{ route('printers.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4"
                          x-data="{ 
                        ...
                        <div class="col-span-1 md:col-span-2 lg:col-span-1">
                             <div class="relative">
                                ...
                                <input type="text" name="search" placeholder="SEARCH BY BRAND, MODEL, IP..." 
                                       class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white text-gray-900 placeholder-gray-500 uppercase font-semibold text-xs h-10"
                        ...
                        <div>
                            <select name="type" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white text-gray-900 font-semibold text-xs h-10">
                                ...
                                <option value="PORTABLE PRINTER" {{ request('type') == 'PORTABLE PRINTER' ? 'selected' : '' }}>PORTABLE PRINTER</option>
                            </select>
                        </div>
                        ...
                        <div class="col-span-1 md:col-span-3 lg:col-span-6 flex items-center gap-2">
                             <a href="{{ route('printers.index', array_merge(request()->query(), ['status' => 'Available'])) }}" 
                         ...
                         @if(request('status') && in_array('Available', (array)request('status')))
                            <a href="{{ route('printers.index', request()->except('status')) }}" class="ml-2 text-indigo-600 hover:text-indigo-900 text-xs font-bold uppercase transition-colors">CLEAR FILTER</a>
                         @endif
                        </div>
                    </form>
                </div>
                                ...
                                @forelse($printers as $printer)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600">
                                            <a href="{{ route('printers.show', $printer) }}" class="hover:underline">{{ $printer->asset_tag }}</a>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ strtoupper($printer->type) }}</div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $printer->employee->full_name ?? 'N/A' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                {{ $printer->status == 'Assigned' ? 'bg-green-100 text-green-800' : '' }}
                                                {{ $printer->status == 'Available' ? 'bg-blue-100 text-blue-800' : '' }}
                                                {{ in_array(strtolower($printer->status), ['defective', 'condemned', 'not_available', 'disposed']) ? 'bg-red-100 text-red-800' : '' }}
                                                {{ !in_array(strtolower($printer->status), ['assigned', 'available', 'defective', 'condemned', 'not_available', 'disposed']) ? 'bg-gray-100 text-gray-800' : '' }}">
                                                {{ strtoupper($printer->status) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('printers.show', $printer) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                            <a href="{{ route('printers.edit', $printer) }}" class="text-blue-600 hover:text-blue-900 mr-3">Edit</a>
                                            <form action="{{ route('printers.destroy', $printer) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-10 text-center text-gray-500">
                                            ...
                                            <p class="text-base font-medium text-gray-900">No printers found</p>
                                                 <p class="text-sm text-gray-500 mt-1">Try adjusting your search or add a new printer.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
