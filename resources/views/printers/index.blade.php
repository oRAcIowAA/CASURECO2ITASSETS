        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- LIST VIEW -->
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <form method="GET" action="{{ route('printers.index') }}" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        ...
                        <div class="col-span-1 md:col-span-3 lg:col-span-6 flex items-center gap-2">
                             ...
                             @if(request('status') === 'Available')
                                <a href="{{ route('printers.index', request()->except('status')) }}" class="ml-2 text-indigo-600 hover:text-indigo-900 text-xs font-bold uppercase">CLEAR FILTER</a>
                             @endif
                        </div>
                    </form>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Brand / Model / IP</th>
                                ...
                                @forelse($printers as $printer)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ $printer->brand }}</div>
                                            <div class="text-xs text-gray-500">{{ $printer->model }}</div>
                                            @if($printer->ip_address)
                                                <div class="text-xs text-gray-400 font-mono mt-0.5">{{ $printer->ip_address }}</div>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ strtoupper($printer->group ?? 'N/A') }}</div>
                                            <div class="text-xs text-gray-500">
                                                @php
                                                    $locParts = array_filter([$printer->department, $printer->division]);
