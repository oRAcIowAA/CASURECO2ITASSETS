                ...
                <form method="GET" action="{{ route('qr-assets.index') }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                        ...
                        <div>
                            ...
                            <select name="group" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white text-gray-900 font-semibold text-xs h-10">
                                ...
                                @foreach($groups ?? [] as $id => $name)
                                    <option value="{{ $id }}" {{ request('group') == $id ? 'selected' : '' }}>{{ strtoupper($name) }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div x-data="{
                            ...
                            <button type="button" @click="open = !open" 
                                class="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm h-10 transition-all duration-200">
                                <span class="block truncate uppercase font-semibold text-xs text-gray-700" x-text="selectedStatuses.length ? selectedStatuses.join(', ').toUpperCase() : 'ALL STATUSES'"></span>
                                <span class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                                    <svg class="h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" />
                                    </svg>
                            ...
                            <div x-show="open" 
                                ...
                                x-transition:enter-start="opacity-0 scale-95"
                                x-transition:enter-end="opacity-100 scale-100"
                                class="absolute hide-scrollbar z-50 mt-1 w-full bg-white shadow-xl max-h-60 rounded-xl py-2 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-xs">
                                
                                <div @click="toggleAll()" class="px-3 py-2 flex items-center justify-between font-bold text-gray-900 cursor-pointer hover:bg-gray-50 transition-colors uppercase">
                                ...
                                <div class="space-y-1">
                                    <template x-for="status in availableStatuses">
                                        <div @click="toggleStatus(status)" class="px-3 py-2 flex items-center cursor-pointer hover:bg-gray-50 transition-colors uppercase">
